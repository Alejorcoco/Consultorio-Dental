...
import { Search, Plus, Edit, User, Filter, Stethoscope, Phone, AlertTriangle, LayoutGrid, List, ArrowDownAZ, ArrowUpAZ, Clock, Calendar, HelpCircle, Info, X, MessageCircle } from 'lucide-react';
...
const PatientsManager: React.FC<PatientsManagerProps> = ({ onNavigate, autoOpenForm = false }) => {
  ...
  const handleWhatsApp = (e: React.MouseEvent, phone: string | undefined) => {
      e.stopPropagation();
      if (!phone) {
          alert('El paciente no tiene un número registrado.');
          return;
      }
      ...
      const formattedNumber = cleanNumber.length === 8 ? `591${cleanNumber}` : cleanNumber;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="p-6 flex-1">
                                    ...
                                    <div className="flex items-start justify-between mb-5">
                                        <div className="flex items-center gap-4">
                                            <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-300 font-bold text-2xl group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors shadow-inner overflow-hidden">
                                                {p.photo ? (
                                ...
                                <div className="p-5 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex items-center justify-end gap-3">
                                    {p.phone && (
                                        <button 
                                            onClick={(e) => handleWhatsApp(e, p.phone)}
                                            className="p-3 text-slate-400 hover:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-xl transition-colors"
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-6 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-14 h-14 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-300 font-bold text-lg shadow-sm overflow-hidden">
                                                        {p.photo ? (
                                                            <img src={p.photo} alt={p.firstName} className="w-full h-full object-cover" />
                                                        ) : (
                                                            <span>{p.firstName.charAt(0)}{p.lastName.charAt(0)}</span>
